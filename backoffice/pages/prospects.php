...
if($_POST['Submit'] == "Save Changes"){
	foreach($_POST as $k => $v){
		if($k != 'Submit'){
			...
			$app->db->update('lmg_prospects',$updateData);
		}
	}
}
	...
	<div class="row">
		<div class="col-lg-4 col-md-4">
			...
			for($mbr=0;$mbr<$dCount;$mbr++){
				?>
				<div>
					<a title="Edit Member" class="icon-edit" href="<?=$_SERVER['PHP_SELF'];?>?url=prospects&id=<?=$directory[$mbr]['mid'];?>"></a>
					<a title="Delete Member" class="icon-remove deleteProspect" href="javascript:void()" id="<?=$directory[$mbr]['mid'];?>"></a>
		...
		if($mbrID != ''){
			?>
			<h3>Edit Prospect (<?=$editMember[0]['mbr_company'];?>)</h3>
					...
					if($fK != 'mid' && $fK != 'last_updated'){
						switch($fK){
							...
							case 'mbr_state':
								?>
								<label><b><?=str_replace("mbr_","",$fK);?></b></label>
								<?=$app->getStateDropdown($fK,$app->states_arr,$editMember[0]['mbr_state']);?>
								<?
								break;
			...
			<?
				...
				$add_mbr = $app->db
					->rawQuery('SHOW columns FROM lmg_prospects');
				$mbrCt = count($add_mbr);
			?>
			<h3>Add Prospect</h3>
				...
				<?
				for($f=0;$f<$mbrCt;$f++){
					if($add_mbr[$f]['Field'] != 'mid' && $add_mbr[$f]['Field'] != 'last_updated'){
				...
				?>
				<p>&nbsp;</p>
				<input class="btn pull-right" type="submit" name="Submit" value="Save Prospect" />
				</form>
			<?
		}
		?>

...
if($_POST['Submit'] == "Save Member"){
	...
	$mbrID2 = $app->db->insert('lmg_users',$insData);
}
...
if($_POST['Submit'] == "Save Changes"){
	foreach($_POST as $k => $v){
		if($k != 'Submit'){
			...
			$app->db->update('lmg_members',$updateData);
		}
	}
}
...
<div class="container">
	<div class="row">
		<div class="col-lg-4 col-md-4">
			...
			for($mbr=0;$mbr<$dCount;$mbr++){
				...
				<div>
					...
					<a title="Delete Member" class="icon-remove deleteMember" href="javascript:void()" id="<?=$directory[$mbr]['mid'];?>"></a>
		...
		if($mbrID != ''){
			?>
			<h3>Edit Member (<?=$editMember[0]['mbr_company'];?>)</h3>
				<form action="<?=$_SERVER['PHP_SELF'];?>?url=members&id=<?=$mbrID;?>" method="post" name="EditMember">
				<?
					...
					if($fK != 'mid' && $fK != 'last_updated'){
						switch($fK){
							...
							case 'mbr_state':
								?>
								<label><b><?=str_replace("mbr_","",$fK);?></b></label>
								<?=$app->getStateDropdown($fK,$app->states_arr,$fV);?>
								<?
								break;
							...
							case 'date_joined':
								?>
								<label><b><?=str_replace("mbr_","",$fK);?></b></label>
								<input class="datepicker" type="text" name="<?=$fK;?>" value="<? echo $fV;?>" />
								<?
								break;
							...
							case 'mbr_type':
								...
								<label><b><?=str_replace("mbr_","",$fK);?></b></label>
								<select name="<?=$fK;?>">
									<option value="Member Firm"<? echo ($fV == 'Member Firm' ? ' selected' : ''); ?>>Member Firm</option>
									<option value="Industry Partner"<? echo ($fV == 'Industry Partner' ? ' selected' : ''); ?>>Industry Partner</option>
								</select>
								<?
								break;
							...
							default:
								?>
								<label><b><?=str_replace("mbr_","",$fK);?></b></label>
								<input type="text" name="<?=$fK;?>" value="<? echo $fV;?>" />
								<?
								break;
			...
			<?
				...
				$add_mbr = $app->db
					->rawQuery('SHOW columns FROM lmg_members');
				$mbrCt = count($add_mbr);
			?>
			<h3>Add Member</h3>
				<form action="<?=$_SERVER['PHP_SELF'];?>?url=members" method="post" name="AddMember">
				<?
				for($f=0;$f<$mbrCt;$f++){
					if($add_mbr[$f]['Field'] != 'mid'){
						switch($add_mbr[$f]['Field']){
							...
							case 'mbr_type':
								?>
								<label><b><?=str_replace("mbr_","",$add_mbr[$f]['Field']);?></b></label>
								<select name="<?=$add_mbr[$f]['Field'];?>">
									<option value="Member Firm">Member Firm</option>
				...
				?>
				<p>&nbsp;</p>
				<input class="btn pull-right" type="submit" name="Submit" value="Save Member" />
				</form>
			<?
		}
		?>
